...
class EEG_antibias_filter():
    ...
    def anti_bias(self, data): # 論文 p.11
    ...
    def trigger_condition(self, data_of_trigger):
        ...
        if data_of_trigger>= 249:
            if   self.act == 'AO':
                ...
                elif (self.trigger_condition_if(data_of_trigger, 'right', 250)): return True
            elif self.act == 'MI':
...
# 1204 寫入 txt 檔案紀錄 (list先左在右)
# 1206 確認哪些是處理過的，並指定一筆trigger有誤的
...
##### [程式用途：分析並畫出ERD] ##### 論文 p.13
# 輸入  ：1.npy                 / 資料夾：FILE_PATH
# 輸出01：ERD.png               / 資料夾：path1(SAVE_PATH_0+日期)
# 輸出02：antibias_list.txt     / 資料夾：path1
# 輸出03：ERDS_data_left.npy    / 資料夾：path1

# 註：12a_ERDs_0307_oneS2.py、12a_ERDs_1127_oneS.py
# 為trigger設定錯誤的版本
...
# %% #  
    ...
    for name in SUBJECT_NAMES:
        ...
        for band in range(len(FRERUENCY_LIST)): # len(FRERUENCY_LIST)
            ...
            # SAVE_TXT_PATH   = f'./2_ERD_plot_30trials/{TEAM}/{name}/'
            check_decoder()         
...
#######################
                ...
                for npy_data_path in npy_data_path_All:
                    ...
                    if now_date == '2023_11_27_1623': continue
                    if now_date == '2024_01_23_1456': continue
                ###
...
# import subprocess
# subprocess.run(["python", "./12a_ERDs_1127_oneS.py"])
# subprocess.run(["python", "./12a_ERDs_0307_oneS2.py"])
# print ("time:", time.process_time())
